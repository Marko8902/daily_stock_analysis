...
- [新功能] 新增 Synology Chat 通知渠道（标准 Incoming Webhook，`SYNOLOGY_CHAT_WEBHOOK_URL`），支持本地、Docker 与 GitHub Actions env 映射，超长消息按 `SYNOLOGY_CHAT_MAX_BYTES`（默认 4000 字节）自动分条发送以规避 code 410 限制。
